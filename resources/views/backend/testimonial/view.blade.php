                                ...
                                <tbody>
                                    @if ($allData->count() > 0)
                                        @foreach ($allData as $key => $data)
                                            <tr>
                                                <td>{{ $key + 1 }}</td>
                                                <td>{{ Illuminate\Support\Str::limit($data->name, 15) }}
                                                    <sub>{{ Illuminate\Support\Str::limit($data->designation, 15) }}</sub>

                                                </td>

                                                <td>{{ Illuminate\Support\Str::limit($data->message, 15) }}</td>
                                                <td>
                                                    <img style="height: 50px; object-fit:contain;"
                                                        src="{{ asset('uploads/testimonial/' . $data->image) }}"
                                                ...
                                                <td>
                                                    <a href="{{ route('edit.testimonial', $data->id) }}"
                                                        class="btn btn-outline-secondary btn-sm edit" title="Edit">
                                                        <i class="fas fa-pencil-alt"></i>
                                                    </a>
                                                    <a id="delete" href="{{ route('delete.testimonial', $data->id) }}"
